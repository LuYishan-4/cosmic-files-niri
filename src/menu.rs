...
pub fn menu_bar<'a>(
    ...
    // NyxNiri intentionally collapses the traditional File/Edit/View/Sort
    // menubar into one overflow capsule. All upstream actions stay available,
    // but the window no longer looks like a conventional desktop menubar app.
    let items = vec![
        ...
        menu::Item::CheckBox(fl!("show-details"), None, config.show_details, Action::Preview),
        menu_button_optional(
            fl!("gallery-preview"),
            Action::Gallery,
            selected_gallery > 0,
